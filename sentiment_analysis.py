...
if __name__ == '__main__':
    ...
    #searchQuery = 'amazonas OR biodiversidad OR eco OR ecofriendly OR ecologico OR OR economiacircular OR energiasrenovables OR MedioAmbiente OR plasticfree OR reciclaje OR renovables OR sostenible OR cambioclimatico OR contaminacion OR emergenciaclimatica OR plastico" + " -filter:retweets"'  # this is what we're searching for
    ...
    until_date = '2019-09-18'

    other_tweets = []
    if os.path.exists(tweets_filename) is True:

        with open(tweets_filename, 'r') as f:
            for line in f:
                other_tweets.append(json.loads(line))

    else:
        other_tweets = twitter_client.get_tweets_search_more_pages(searchQuery, tweetsPerQry, language, until_date, sinceId, maxTweets, tweets_filename)

    # se convierte a DataFrame
    user_list = tweet_analyzer.convert_tweets_to_data_frame(other_tweets, False)
    ...
    df = df.sort_values(by='user_name_count', ascending=False, na_position="last")
    data = go.Data([
        go.Bar(
            x=df.index,
    ...
    layout = go.Layout(
        title="Usuarios con mayor cantidad de Tweets"
    )
    ...
    # se grafica un scatter plot con los datos de polaridad de los usuarios seleccionados
    df_general['created_at'] = np.array([
              datetime.strptime(datetime.strptime(item, "%a %b %d %H:%M:%S %z %Y").strftime("%m/%Y"), "%m/%Y")
              if isinstance(item, str)
              else datetime.strptime(item, "%a %b %d %H:%M:%S %z %Y").strftime("%m/%Y") for item in df_general['created_at']])

    df_general.sort_values(by='created_at', ascending=True)

    fig = px.scatter(df_general.sort_values(by='created_at', ascending=True), x="created_at", y="polarity",
                     color="user");
    fig.update_layout(title_text='Polaridad general', xaxis_rangeslider_visible=False,
                      yaxis=go.layout.YAxis(title_text="Polaridad",),
                      xaxis=go.layout.XAxis(title_text="Fecha creación", )
                      );
    fig.show()
    ...
    negativos = 0;
    neutros = 0;
    for item in df_general['polarity']:
        if item < 0:
            negativos = negativos + 1;
        elif item > 0:
            positivos = positivos + 1;
    ...
    values = [positivos, negativos, neutros]
